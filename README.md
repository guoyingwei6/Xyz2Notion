...
- **2026-08-04**：AI 视图新增独立的 `增强状态` 与 `增强 Provider`，明确区分摘要/章节/思维导图和语音转写的处理状态与实际通道。
- **2026-08-04**：增强队列增加每两小时一次的遗留文字稿检查，避免转写完成后因事件遗漏而长期停在“已转写”。
- **2026-08-04**：新增独立的 `Retry Failed Episode AI` 失败队列，每两小时只检查并重试 `可重试失败`，不抢占普通候选；ASR 成功后仍立即接续摘要与思维导图。
- **2026-08-04**：`可重试失败` 按失败阶段自动回到对应队列；只从已保存检查点继续，最多累计重试 3 次，不重复已完成的转写或摘要。
- **2026-08-04**：Episode AI 视图新增 `人工请求重试` 复选框。勾选后由两小时兜底队列优先处理，并根据检查点自动判断从 ASR、摘要或 Notion 发布阶段继续；成功或再次失败后自动清除请求，无需修改 GitHub 代码。
- **2026-08-04**：`人工请求重试` 同时支持普通待处理、已转写和已增强单集的优先处理；排队中/转写中的已有任务只查询原 task ID，不重复提交，仍在处理中的请求会保留勾选。
