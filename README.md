...
- **2026-09-07**：修复旧版热力图未被识别导致同年显示两张图的问题；兼容旧文件名并安全去重，保留往年热图和个人图片，更新失败时不清理旧图。
- **2026-09-05**：修复首次统计漏计、热力图误删及内部封面覆盖问题；完善 ASR 断点恢复、处理限额与摘要批次隔离，避免不确定写入被重复执行，失败状态如实上报。513 项测试及 CI 全部通过。
