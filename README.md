...
- **小宇宙同步**：同步订阅播客、作者、实际播放历史、播放进度、待听播放列表、喜欢和收藏；只把真实播放纳入收听统计。
- **Notion 一体化**：自动维护 Podcast、Episode、Author、统计、文字稿和思维导图等数据库，并生成可直接浏览的主页视图、排行、趋势图和热力图。
- **AI 转写与增强**：语音转写按“百炼 Paraformer → SiliconFlow → 本地 Whisper”降级；摘要、章节和思维导图按“SiliconFlow → 本地 Qwen”降级。
...
如果某期进入 `可重试失败`，独立的 `Retry Failed Episode AI` 每两小时只检查失败检查点，
按 `resume_state` 只重试原失败阶段；未勾选人工请求的普通待处理、已转写和已增强单集不会被
这个失败分支选中。
ASR 重试成功后会触发增强队列，立即继续摘要、章节和思维导图；摘要/发布失败也由同一失败队列
重试，不会重新转写。每个阶段每次最多处理 2 期。

如果想让某个单集优先处理，即使它还没有失败，也可以在 Episode 的“转写文本”或
“AI总结与思维导图”视图勾选 `人工请求重试`。下一次两小时兜底运行会先消费这些请求：
待处理从 ASR 开始，已转写从增强开始，已增强从 Notion 发布开始；排队中/转写中的任务只查询
已有检查点，不会重复提交。失败单集则按已有文字稿和摘要检查点继续，最终失败才会重新打开。
处理结束后复选框会自动取消；如果再次失败，会回到普通的 `可重试失败` 兜底流程。
...
一个单集同时命中多个状态时取更高优先级。这个顺序只决定普通候选先后，不会放宽
“播放至少 120 秒、收藏或喜欢”的门槛；收藏或喜欢的单集即使从未播放也会进入候选。
人工勾选是显式优先请求，会绕过这条自动门槛，但 `Skip AI` 仍然是硬性排除开关。
...
- **AI总结与思维导图**：直接列出 Episode 数据库中 `增强状态=已完成` 的单集，按 `总结完成时间` 从新到旧排序；视图显示独立的 `增强状态` 与 `增强 Provider`，不再把 ASR 状态当成摘要状态。点击 `Name` 就能打开该单集页面，摘要、章节和思维导图属于同一次增强流程，会在同一单集页面中一起发布。

底层的独立“思维导图”数据库仍保留，用于存储可检索的脑图 JSON、Mermaid 和 Episode 关系；它不是第二条 AI 任务链，也不会再单独占用主页展示位。

`增强 Provider` 与 `增强状态`描述摘要、章节和思维导图共用的 AI 增强流程；`ASR Provider` 与 `ASR Status` 只描述语音转文字。
...
- **2026-08-04**：AI 视图改用 Episode 作为数据源，新增独立的增强状态/Provider，并支持人工优先重试；转写成功后会立即接续摘要、章节和思维导图。
- **2026-08-04**：百炼 ASR 增加 `paraformer-v1 → paraformer-v2 → paraformer-mtl-v1` 的内部降级，之后才进入 SiliconFlow 和本地 Whisper。
- **2026-08-03**：统计日期统一按 UTC+8 计算，趋势图和热力图的日期不再受 GitHub Runner 时区影响。
- **2026-07-31**：AI 队列改为事件驱动，移除听悟网页 Cookie 链路，保留检查点恢复和本地模型兜底。

完整的功能、工作流和数据结构变更见 [CHANGELOG.md](CHANGELOG.md)。
