...
按 `resume_state` 只重试原失败阶段；普通待处理、已转写和已增强单集不会被这个工作流选中。
ASR 重试成功后会触发增强队列，立即继续摘要、章节和思维导图；摘要/发布失败也由同一失败队列
重试，不会重新转写。每个阶段每次最多处理 2 期。

如果某期进入 `最终失败`，不需要改代码或手动编辑检查点。打开 Episode 的“转写文本”或
“AI总结与思维导图”视图，勾选 `人工请求重试`，下一次两小时兜底运行会读取已有检查点：
没有文字稿则从 ASR 开始，有文字稿但没有摘要则从增强开始，已有摘要则只重试 Notion 发布。
请求被消费后复选框会自动取消；如果再次失败，会回到普通的 `可重试失败` 兜底流程。
