...
不受固定半小时窗口影响。`可重试失败` 会按保存的 `resume_state` 回到原失败阶段，
在下一次相关事件运行时重试；摘要/发布阶段另有每两小时增强兜底，转写阶段只有在
存量开关开启时才有每两小时转写兜底。每次队列最多处理 2 期，同一期累计重试 3 次后
转为最终失败。
...
下一次相关事件运行会重试；摘要/发布阶段还会由两小时增强兜底接手，转写阶段在存量
开关开启时才由两小时转写兜底接手，也可以手动运行对应工作流。百炼认证、
