...
- [x] 接入总工作流状态机后模拟验证失败状态的跨 Action 恢复（P10）。
...
- [x] 方言明显合成文字稿下游契约。
- [x] 多人对话合成文字稿下游契约。
- [x] 背景音乐或远场录音合成文字稿下游契约。
- [x] 超过 1 小时的合成长单集分块。
- [ ] 使用真实六类音频评估 ASR 准确率和页面效果（等待 Provider 与 Notion 凭证）。

### 故障注入

- [x] 小宇宙 Refresh Token 错误。
- [x] Notion Token 错误。
- [x] 听悟 Cookie 过期。
- [x] 听悟内部接口字段改变。
- [x] SiliconFlow 429。
- [x] SiliconFlow 模型 404。
- [x] 音频 URL 失效或指向私网。
- [x] GitHub Action 中途取消并从检查点恢复。
- [x] Notion 429/529。
- [x] AI JSON 无效并只修复一次。
...
- [x] 迁移指南。
- [x] Changelog。
- [x] 发布 [v0.1.0 预发布版](https://github.com/guoyingwei6/Xyz2Notion/releases/tag/v0.1.0)，
  附 wheel、sdist 和 SHA-256。
...
- 当前阶段：`P12 端到端 QA 与发布`
- 下一项：用户配置 GitHub Secrets 后，按 P2→P12 顺序完成全部真实账户验收项
